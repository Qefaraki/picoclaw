...
func (ke *KnowledgeExtractor) ExtractAndConsolidate(ctx context.Context, userMsg, assistantMsg, sessionKey string) {
...
func (ke *KnowledgeExtractor) consolidateFact(ctx context.Context, fact ExtractedFact) error {
	...
	existing, err := ke.store.SearchKnowledge(ctx, fact.Fact, 3)
	if err != nil {
		// If search fails, just add as new
		return ke.store.IndexKnowledge(ctx, "", fact.Fact, fact.Category)
	}
	...
	case "UPDATE":
		...
		return ke.store.IndexKnowledge(ctx, "", newFact, fact.Category)
